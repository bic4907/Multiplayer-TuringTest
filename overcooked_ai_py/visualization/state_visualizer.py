...
class StateVisualizer:
    ...
    DEFAULT_VALUES = {
        ...
        "cooking_timer_font_size": 40, # # if set to None use cooking_timer_font_path
        "cooking_timer_font_path": roboto_path, 
        "cooking_timer_system_font_name": None,
        "cooking_timer_font_color": (255, 0, 0), # red
        "grid": None,
        "background_color": (0, 0, 0), # color of empty counter
    ...
    def default_hud_data(state, **kwargs):
        ...
        result = {"all_orders":[r.to_dict() for r in state.all_orders[:2]], "score":kwargs['score'], "time_left":kwargs['time_left']}
    ...
    def render_state(self, state, grid, hud_data=None, action_probs=None):
        ...
        if self.is_rendering_hud and hud_data:
            ...
            # Custom
            hud_surface = pygame.surface.Surface((hud_width, self.hud_margin_top + 1.5 * self.hud_line_height + self.hud_margin_bottom))
        ...
        for key, obj in objects.items():
            ...
            if obj.name == "soup" and grid[y_pos][x_pos] == POT:
                if obj._cooking_tick != -1 and (obj._cooking_tick <= obj.cook_time or self.show_timer_when_cooked):
                    ...
                    WIDTH = int(self.tile_size * 0.98)
                    _percentage = obj._cooking_tick / obj.cook_time

                    if _percentage <= 0.999:
                        pygame.draw.rect(surface, [255, 0, 0], pygame.Rect(tile_pos_x, tile_pos_y + WIDTH * 0.65,
                                                                           int(WIDTH * _percentage),
                                                                           text_surface.get_height() * 0.2))

                        pygame.draw.rect(surface, [255, 0, 0], pygame.Rect(tile_pos_x, tile_pos_y + WIDTH * 0.65,
                                                                           WIDTH,
                                                                           text_surface.get_height() * 0.2), 2)
    ...
    def _render_hud_data(self, surface, hud_data):
        def hud_text_position(line_num):
            ...
            # Custom
            return (self.hud_margin_left + (surface.get_width()/2.8) * line_num, self.hud_margin_top)
        ...
        for hud_line_num, (key, value) in enumerate(self._sorted_hud_items(hud_data)):
            ...
            if key not in ["time_left"]:
                ...
                text_surface = self.hud_font.render('      '+str(int(value/60))+' : '+str(int(value%60)), True, font_color)
            # text_surface = self.hud_font.render(hud_text, True, self.hud_font_color)
